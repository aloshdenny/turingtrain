...
def main():
    ...
    # Inject ONNX nodes to enforce monotonicity and T05 physical lower bound post-regressor
    ...
    # Define a lower bound of 300.0 Kelvin (26.85 °C) to prevent non-physical extrapolation
    clip_min = numpy_helper.from_array(np.array([300.0], dtype=np.float32), name="clip_min")
    raw_graph.initializer.append(clip_min)
    ...
    # 1.b Clip d0 to be at least 300.0 K (26.85 °C)
    node_clip_d0 = helper.make_node(
        'Clip',
        inputs=['d0', 'clip_min'],
        outputs=['d0_clipped'],
        name='clip_d0'
    ...
    # 4. Concatenate d0_clipped and d_rest_clipped
    node_concat = helper.make_node(
        'Concat',
        inputs=['d0_clipped', 'd_rest_clipped'],
    ...
    raw_graph.node.extend([node_slice_d0, node_clip_d0, node_slice_drest, node_relu, node_concat, node_matmul])
