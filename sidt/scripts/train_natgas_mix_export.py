...
def main():
    parser = argparse.ArgumentParser(description="Train and export SIDT idt_400K_s model for natural gas mixture.")
    parser.add_argument("--input", type=str, default="model_training/sidt/sidt_selfies_natgas_mix.dat", help="Path to input dataset .dat file")
    parser.add_argument("--out_dir", type=str, default="sidt/models/natgas_mix", help="Directory to save exported ONNX model")
    ...
    print(f"Loading natural gas mixture dataset from: {input_path}")
    df = pd.read_csv(input_path, sep='\t', comment='#')

    # Feature columns: 6 mole fractions + pressure_pa + temperature_K + phi + egr_fraction
    feature_cols = [
        'cpnt_mole_frac_1', 'cpnt_mole_frac_2', 'cpnt_mole_frac_3',
        'cpnt_mole_frac_4', 'cpnt_mole_frac_5', 'cpnt_mole_frac_6',
    ...
    pipeline = Pipeline([
        ...
        ('regressor', RandomForestRegressor(n_estimators=100, max_depth=13, min_samples_leaf=2, random_state=42, n_jobs=-1))
    ])
    ...
    sample_pred = float(sample_res.item() if sample_res.size == 1 else sample_res[0])
    print(f"\nSample ONNX Inference (P = 10 bar / 1.0 MPa, T = 1000 K, φ = 1.0, EGR = 0.0, equimolar 6-mix):")
